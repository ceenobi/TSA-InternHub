...
const AnnouncementCard = memo(function AnnouncementCard({
	...
	useEffect(() => {
		...
		if (fetcher.data.success) {
			...
			toast.error(fetcher.data.message || "Action failed");
		}
	...
	const handleDelete = () => {
		...
		fetcher.submit(
			{ intent: "delete-announcement", announcementId: announcement._id },
			{
				method: "post",
	...
	const handleTogglePin = () => {
		fetcher.submit(
			{ intent: "toggle-pin", announcementId: announcement._id },
			{
				method: "post",
				action: "/announcements",
				encType: "application/json",
						...
						{isAdmin && (
							<div className="flex items-center gap-1">
								<button
									...
									disabled={isMutating}
									className="p-1 rounded-sm text-muted-foreground hover:bg-muted hover:text-foreground transition disabled:opacity-50"
								...
								<button
									...
									disabled={isMutating}
									className="p-1 rounded-sm text-muted-foreground hover:bg-muted hover:text-destructive transition disabled:opacity-50"
									title="Delete"
