...
- [x] **CodeQL fix (PR #44):** Added `permissions: { contents: read }` to `.github/workflows/ci.yml` to satisfy GitHub CodeQL's `actions/missing-workflow-permissions` alert. All 4 alerts auto-closed. Verified 0 open code-scanning alerts.
- [x] **Vercel serverless optimization (PR #45):** Converted 12 heavy dashboard routes from server `loader` to hybrid `loader` + `clientLoader` pattern. Server loaders now return only auth data; client loaders fetch page data via 12 new `/api/v1/*` API endpoints using TanStack Query. Added 6 client query files (`dashboard.ts`, `tasks.ts`, `projects.ts`, `calendar.ts`, `auditlogs.ts`, `announcements.ts`). Reduces serverless function invocations and cold starts on Vercel. 31 files changed; typecheck + lint clean.
- [x] **Automatic revalidation (PR #50):** Root cause: `ensureQueryData` resolves stale cached data (60s `staleTime`) and there were no active TanStack Query observers, so `invalidateQueries` was a no-op and post-mutation UI never refreshed without a reload. Fixes: (1) `app/.server/action/auth.ts` — `updateProfileRequest`/`updateAvatarRequest`/`onboardUser` now forward refreshed Better Auth `Set-Cookie` headers via `headers.getSetCookie()` (previously discarded → stale user ≤5 min); settings profile form resets on success. (2) Converted `Await`-based routes to active observers: announcements → `useInfiniteQuery`, tasks / task-stats / task-submissions / projects layout+all+records / members cohorts → `useQuery`, with browser-safe query options in `app/queries/{announcements,tasks,projects}.ts` (no `request.headers`; queryKey shapes preserved for prefix invalidation). (3) Added `queryClient.invalidateQueries({ queryKey: [...] })` in every mutation success effect: create/delete/pin announcements, activate-stage, submit-task, create/update/delete project (`projects-layout`/`projects`/`scoreboard`), add/update cohort (`cohorts`/`projects-layout`). (4) Hub native `confirm()` → shared `ConfirmDialog`. Files: 22 changed. Verified: typecheck clean, lint 0 errors, 481 tests pass.
